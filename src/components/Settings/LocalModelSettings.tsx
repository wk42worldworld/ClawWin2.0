...
export const LocalModelSettings: React.FC<LocalModelSettingsProps> = ({ onSaved }) => {
  ...
  const [installProgress, setInstallProgress] = useState<{ progress: number; downloadedBytes?: number; totalBytes?: number; speed?: number; status: string } | null>(null)
        ...
        <div className="local-model-download-bar">
          <div className="local-model-download-info">
            ...
            {installProgress.progress >= 0 && (
              <span>{installProgress.progress}%{installProgress.downloadedBytes && installProgress.totalBytes ? ` (${formatBytes(installProgress.downloadedBytes)}/${formatBytes(installProgress.totalBytes)})` : ''}{installProgress.speed ? ` ${formatBytes(installProgress.speed)}/s` : ''}</span>
            )}
          </div>
